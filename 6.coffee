...
execute = ( cmds, f1, f2, f3 )->
	lights = for [0...1000]
		for [0...1000]
			0

	reg_on = /turn on (\d+),(\d+) through (\d+),(\d+)/
	reg_off = /turn off (\d+),(\d+) through (\d+),(\d+)/
	reg_toggle = /toggle (\d+),(\d+) through (\d+),(\d+)/

	exec = ( l, t, r, b, f )->
		for y in [+t..+b]
			for x in [+l..+r]
				lights[y][x] = f lights[y][x]
		return
	...
	for cmd in cmds
		switch
			when res = reg_on.exec cmd
				exec res[1], res[2], res[3], res[4], f1
			when res = reg_off.exec cmd
				exec res[1], res[2], res[3], res[4], f2
			when res = reg_toggle.exec cmd
				exec res[1], res[2], res[3], res[4], f3
			else
				_log 'invalid input', cmd
	sum()
