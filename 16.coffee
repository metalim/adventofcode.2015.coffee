...
find_aunt_strict = ( aunts, gift )->
	for id, a of aunts
		matched = yes
		for key, val of a
			if gift[key] isnt val
				matched = no
				break
		if matched
			return id
...
find_aunt_adjusted = ( aunts, gift )->
	for id, a of aunts
		matched = yes
		for key, val of a
			matched and= switch key
				when 'cats', 'trees'
					gift[key] < val
...
try
	...
	_log find_aunt_strict aunts, gift
	_log find_aunt_adjusted aunts, gift
catch e
	_log e
